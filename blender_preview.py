...
def set_preview_render(scene, preview_cfg):
    ...
    scene.cycles.samples = int(preview_cfg.get("samples", 16))

    w, h = preview_cfg.get("resolution", [512, 512])
...
# ---- camera fit helpers (same logic as render, but only for one angle) ----
...
def main():
    ...
    out_path = preview_cfg.get("out_path", os.path.join(out_dir, "_preview.png"))
    ...
    scene.render.fps = int(cfg.get("animation", {}).get("fps", 24))
    set_preview_render(scene, preview_cfg)
    ...
    # animation
    anim_name = cfg.get("animation", {}).get("name", "(static)")
    anim_info = apply_animation_exclusive(anim_name)

    step = max(1, int(cfg.get("animation", {}).get("step", 1)))
    if anim_info:
    ...
    # choose preview frame by pose percent
    pose_percent = int(preview_cfg.get("pose_percent", 0))
    if anim_info:
        frame = fs + int(round((fe - fs) * (pose_percent / 100.0)))
    else:
        frame = 0

    # For preview: sample a few frames around the animation to compute safe distance (fast)
    if anim_info:
        total = max(1, fe - fs + 1)
        sample_cnt = min(20, total)
        sample_step = max(step, int(math.ceil(total / max(1, sample_cnt))))
        sample_frames = list(range(fs, fe + 1, sample_step))
        if not sample_frames:
            sample_frames = [frame]
    else:
        sample_frames = [frame]

    yaw = float(preview_cfg.get("yaw", 0.0))
    pitch = float(preview_cfg.get("pitch", 10.0))
    ...
    if cam.type == "PERSP":
        ...
        for fr in sample_frames:
            ...
            t = t + target_offset_v
            needed_d = max(needed_d, required_dist_for_bounds(t, b, dir_n, right, up, tanx, tany))

        dist = needed_d * fit_margin * dist_mult
    else:
        # ortho: use existing ortho_scale or just keep default; preview is mostly for composition
        user_scale = float(camera_cfg.get("ortho_scale", 3.0))
        cam.ortho_scale = user_scale
        dist = max(1.0, cam.ortho_scale * 2.0)

    # Set exact preview frame
    scene.frame_set(int(frame))
    depsgraph.update()

    b = compute_bounds_eval(ref_mesh_objects, depsgraph)
    if arm_obj and root_pb:
        target = (arm_obj.matrix_world @ root_pb.head) + target_offset_v
    else:
        if b:
            mins, maxs = b
            target = (mins + maxs) * 0.5 + target_offset_v
        else:
            target = target_offset_v

    cam_obj.location = target + (dir_n * dist)
    look_at(cam_obj, target)
    cam_obj.rotation_euler.rotate_axis("Z", roll_rad)

    # visibility for preview (combined)
    names = [x["name"] for x in ordered]
    visible = [n for n in names if modes.get(n) == "VISIBLE"]
    holdout = [n for n in names if modes.get(n) == "MASK"] if bool(preview_cfg.get("preview_holdouts", True)) else []

    original_mats = {o.name: [s.material for s in o.material_slots] for o in mesh_objects}
    holdout_mat = create_holdout_material()
    apply_pass_state(mesh_objects, set(visible), set(holdout), holdout_mat, original_mats)
